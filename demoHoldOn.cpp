...
void processSpecialKeys(int key, int x, int y) {
	...
	switch(key) {
		...
		case GLUT_KEY_F1:
			if (holdOn != NULL) {
				cout << "Recuperando Estado através do holdOn..." << endl;
				holdOn->goBack(1500);
			}
			break;
	}
...
int main(int argc, char** argv) {
	...
	holdOn = new StateCircularQuee();
	holdOn->addTargetPiece(puppet);
	holdOn->setTargetCamera(camera);
	holdOn->start();
